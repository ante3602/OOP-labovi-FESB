...
//    POST: string sadr�i jedan znak 
{
    ...
    m_length = 1;
    m_cstr[0] = ch;             // string za smje�taj jednog znaka
    m_cstr[1] = '\0';
    return *this;
}
...
//    PRE:  0 <= k < length()
//    POST: vra�a referencu k-tog znaka
...
//    POST: str se �elje izlaznom toku os
{
    return os << str.c_str();
}
...
{
    char ch;
    str = "";    // prazan string, dodavat �emo znak po znak
...
//    PRE:  ulazni tok je otvoren 
//    POST: vi�e znakova zaklju�no s '\n' se o�itava s ulaznog toka
//          i sprema u str ('\n' se ne sprema u str)
{
...
//    POST: dodaje kopiju str stringa na kraj this stringa
{
    ...
    int lastLocation = length();      // polo�aj  '\0' znaka

    // provjeri da li treba pove�ati memoriju
    if (newLength >= m_capacity)
    {
        m_capacity = newLength + 1;
        if (str.length() == 1)  // specijalni slu�aj- dodaje se jedan znak
        {                       
            m_capacity *= 2;
        }
    ...
    m_length = newLength;           // upi�i novu duljinu stringa
...
{
    TString temp;   // napravi string koj sadr�i znak ch
    temp = ch;
    *this += temp;
    return *this;
}
...
//    POST: vra�a string koji se sastoji od lhs dopunjenog s rhs
{
    TString result(lhs);  // kopira lhs u result
    result += rhs;         // dopunjuje rhs
    return result;         // vra�a result
}

TString operator + ( char ch, const TString & str )
//    POST: vra�a string koji se sastoji znaka ch i stringa str
{
    TString result; // napravi string koj sadr�i znak ch
    result = ch;
    result += str;
    return result;
}
...
//    POST: vra�a string koji se sastoji stringa str dopunjenog znakom ch
{
    TString result(str);
    result += ch;
    return result;
...
//    Opis:   vra�a substring duljine len po�ev�i od indeksa pos
//    PRE:  this string sadr�i c0, c1, ..., c(n-1), uz uvjet
//                         0 <= pos <= pos + len - 1 < n.
//    POST: vra�a string koji sadr�i
//                   c(pos), c(pos+1), ..., c(pos+len-1)
{
    ...
    result.m_length = j;              // i zabilje�i duljinu
    return result;
}

int TString::find(const TString & str)  const
//    Opis:   pronalazi pojavu stringa str u this stringu
//            i vra�a indeks prvog znaka. Ako this string ne sadr�i
//            string str, tada vra�a npos.
//    PRE:  this string sadr�i c0, c1, ..., c(n-1)
//          str sadr�i s0, s1, ...,s(m-1)
//    POST: ako je s0 == ck0, s1 == ck1, ..., s(m-1) == ck(m-1) i ako
//          ne postoji j < k0 takovi da je s0 = cj, ...., sm == c(j+m-1),
//              tada vra�a k0;
//              u suprotnonm slu�aju vra�a npos
{
    int len = str.length();
...
//    Opis:   pronalazi prvu pojavu znaka u this stringu i vra�a indeks
//            Ako znak chnije u this stringu, tada vra�a npos.
//    PRE:  this string sadr�i c0, c1, ..., c(n-1)
